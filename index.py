...
class Mamalia(Animal):
    ...
    def tampil(self):
        print(f'Nama hewan: {self.nama}, sifat hewan: {self.sifat}, 
              ukuran: {self.ukuran}, jumlah kaki: {self.kaki}, 
...
class Aves(Animal):
    ...
    def tampil(self):
        print(f'Nama hewan: {self.nama}, sifat hewan: {self.sifat}, 
              ukuran: {self.ukuran}, jumlah kaki: {self.kaki}, 
              bisa jalan: {self.bisa_terbang}, jenis mamalia: {self.jenis_aves}')
class Ayam(Aves):
    def __init__(self, nama, sifat, ukuran, kaki, bisa_terbang, jenis_aves, jenis_ayam, bisa_diadu):
        super().__init__(nama, sifat, ukuran, kaki, bisa_terbang, jenis_aves)
        self.jenis_ayam = jenis_ayam
        self.bisa_diadu = bisa_diadu

# Contoh penggunaan
ayam_pertama = Ayam("Ayam Putih", "Ramah", "Sedang", 2, True, "Burung", "Ayam Kampung", True)
print(f"{ayam_pertama.nama} adalah sejenis {ayam_pertama.jenis_ayam} dan {'bisa' if ayam_pertama.bisa_diadu else 'tidak bisa'} diadu.")